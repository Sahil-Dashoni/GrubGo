...
function OwnerOrderCard({ data }) {
    ...
    const handleVideoUpload = async (orderId) => {
        ...
        formData.append("video", videoFile); // Ensure your backend multer is looking for req.file with name "video"

        try {
            await axios.patch(`${serverUrl}/api/order/upload-video/${orderId}`, formData, {
    ...
    return (
        <div className='bg-white rounded-lg shadow p-4 space-y-4'>
            ...
            <div className='flex justify-between items-center mt-auto pt-3 border-t border-gray-100'>
                ...
                {data.shopOrders.status !== "delivered" && <select className='rounded-md border px-3 py-1 text-sm focus:outline-none focus:ring-2 border-[#ff4d2d] text-[#ff4d2d]' onChange={(e) => handleUpdateStatus(data._id, data.shopOrders.shop._id, e.target.value)}>
            ...
            {!data.shopOrders.videoUrl && data.shopOrders.status !== "delivered" && (
                <div className='mt-3 pt-3 border-t flex flex-col gap-2'>
                    <p className='text-sm font-semibold'>Upload Order Video:</p>
                    <div className='flex items-center gap-3'>
                        <input type="file" accept="video/*" className='text-sm cursor-grabbing' onChange={(e) => setVideoFile(e.target.files[0])} />
                        <button onClick={() => handleVideoUpload(data._id)} className='bg-blue-600 text-white px-3 py-1 rounded text-sm cursor-pointer'>Upload</button>
                    </div>
                </div>
            )}

            {data.shopOrders.videoUrl && (
                <video src={data.shopOrders.videoUrl} controls />
            )}
